...
- **1.1.1** Создать проект `mgsrc/backend`, пакет `fedoc_multigraph`, базовый `app.py`, `setup.py`, конфигурацию подключения к БД.  
  **Ссылки на выполнение:** *(нет)*
- **1.1.2** Подготовить `requirements.txt`, настроить версионирование зависимостей, описать запуск в README backend.  
  **Ссылки на выполнение:** *(нет)*

### 1.2 REST API
- **1.2.1** Реализовать CRUD для строительных блоков `/api/blocks`, включая валидацию входных данных и запись в `mg_blocks`.  
  **Ссылки на выполнение:** *(нет)*
- **1.2.2** Реализовать CRUD для дизайнов `/api/designs`, связь дизайн ↔ блок через `mg.design_to_block`.  
  **Ссылки на выполнение:** *(нет)*
