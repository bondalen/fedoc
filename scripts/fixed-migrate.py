#!/usr/bin/env python3
"""Исправленная миграция данных из JSON в PostgreSQL AGE"""
...
def main():
    ...
    for i, node in enumerate(nodes):
        ...
        # Создать узел с правильным экранированием
        sql = f"""
        SELECT * FROM cypher('common_project_graph', $$
            CREATE (n:canonical_node {props_cypher})
            RETURN id(n) as vertex_id
        $$) as (vertex_id agtype)
        """.replace('{props_cypher}', props_cypher)
        
        result = run_sql(sql)
        if result and 'vertex_id' in result:
            # Извлечь ID из результата
            lines = result.strip().split('\n')
            for line in lines:
                if 'vertex_id' in line and '|' in line:
                    try:
                        age_id = int(line.split('|')[1].strip())
    ...
    for i, edge in enumerate(edges):
        ...
        # Создать ребро с правильным экранированием
        sql = f"""
        SELECT * FROM cypher('common_project_graph', $$
            MATCH (a), (b)
            WHERE id(a) = {from_age} AND id(b) = {to_age}
            CREATE (a)-[e:project_relation {props_cypher}]->(b)
            RETURN id(e) as edge_id
        $$) as (edge_id agtype)
        """.replace('{props_cypher}', props_cypher)
        
        result = run_sql(sql)
        if result and 'edge_id' in result:
            migrated_edges += 1
